...
- **How-it-works vignette** (`Steps.astro` + `.steps-*` in `global.css`): three miniature plates of the real UI (composer typing, agent trace mid-check, verdict list) on a rail that draws itself once on first view; plates and chips enter in sequence. Trace labels reuse `chat.tools.*` — the demo shows only real stages. JS adds the hidden "prep" state only when motion is allowed, so no-JS and reduced-motion users get the settled section; the blinking caret and breathing mark are the only ambient movement, both disabled under reduced motion.
