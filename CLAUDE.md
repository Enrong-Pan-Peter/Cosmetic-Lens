...
- Bilingual. Every user-facing string lives in BOTH `src/i18n/en.json` and `src/i18n/zh.json`. Update both. Fonts are Inter plus Noto Sans SC for body/UI, Playfair Display plus Noto Serif SC for display (`font-display`), and JetBrains Mono for data details (`font-mono`). Never italicize CJK; color the accent word instead.
